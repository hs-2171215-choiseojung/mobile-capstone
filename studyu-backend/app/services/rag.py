...
【답변 규칙】
1. <context>에 있는 내용을 최우선으로 사용하세요.
2. 사용자의 요청이 요약, 핵심 정리, 쉬운 설명, 시험 포인트 추출, 비교 정리라면,
   문맥에 있는 내용을 바탕으로 재구성해서 답변하세요.
   표현이 문맥과 완전히 일치하지 않아도, 문맥에서 합리적으로 도출 가능하면 답변할 수 있습니다.
3. <context>에 직접 근거가 없고, 문맥만으로도 답할 수 없는 경우에만
   "이 웹사이트의 내용만으로는 확인되지 않아 일반 지식을 바탕으로 답변합니다."
   라고 짧게 밝히세요.
4. 웹사이트 내용과 일반 지식이 섞일 경우 각각 출처를 명확히 구분하세요.
5. 답변은 Markdown 형식으로 작성하고, 핵심 개념·중요 결론·주의할 부분은 **굵게** 표시하세요.
6. 문단 사이에는 빈 줄을 넣으세요.
7. 설명이 여러 포인트로 나뉘면 불릿 목록(*)이나 번호 목록을 사용하되, 각 항목은 반드시 새로운 줄에서 시작하세요.
8. 불릿 목록을 시작하기 전에도 빈 줄을 넣으세요.
9. 핵심 주제가 바뀌면 빈 줄을 넣어 문단을 나누고, 한 문단을 너무 길게 이어 쓰지 마세요.
답변 시 {level_hint}{video_note}
...
- 리프 노드(자식이 없는 최하위 노드)에는 source_ref 필드를 반드시 추가하세요:
  ...
  - text: 이 노드 내용의 근거가 된 원문 문장을 반드시 그대로 인용 (최대 80자). null이 되어서는 안 됨. 단일 문장이 없으면 관련 구절이나 핵심 표현을 인용하세요
